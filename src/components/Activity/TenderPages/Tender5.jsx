  ...
  return (
    <div className="tenders-page__info1">
      <div className="container">
        ...
        <span className="enter"></span>
        <p>
           {textInfo?.blocks?.[0]?.description?.slice(0, 4).map((text, idx) => (
          ...
          <span className="enter"></span>
          <span className="bold">{textInfo?.blocks?.[0]?.description?.[7]?.children?.[0]?.text}</span>
          <span className="enter"></span>
           {textInfo?.blocks?.[0]?.description?.slice(8, 19).map((text, idx) => (
            <p key={idx}>{text.children?.[0]?.text}
            <span className="enter"></span></p>
          ))}
        </p>
        <h3 className="bold">{textInfo?.blocks?.[0]?.title3}</h3>
        <span className="enter"></span>
        <h3 className="bold">{textInfo?.blocks?.[0]?.title_num}</h3>
        <span className="enter"></span>
        <h3>{textInfo?.blocks?.[0]?.title_table}</h3>
        <h3 className="enter"></h3>
          {textInfo?.blocks?.[0]?.table_description?.slice(0, 5).map((text, idx) => (
            <p key={idx}>{text.children?.[0]?.text}
            <span className="enter"></span>
            </p>
          ))}
        ...
        <table className="tenders-page__info1-table" border="1">
          <tbody>
            {(() => {
              const rows = [];
              const table = textInfo?.blocks?.[0]?.table?.slice(7, 30) || [];
